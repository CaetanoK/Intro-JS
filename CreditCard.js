...
const validateCred = arr => {
    ...
    let indexArray = []
    // build the Luhn Algorithm
    for(i = 0; i< array.length ; i++){
        if(i%2 === 0){
            indexArray.push(array[i])
        } else if (i%2 === 1 && array[i]>=5) {
            indexArray.push( (array[i]*2)-9 )
        } else if (i%2 === 1) {
            indexArray.push( (array[i]*2) )
        }
    }
    // check for validity
    if (indexArray.reduce((a,b) => a+b,0)%10 === 0){
        switch (arr[0]){
            case 3:
